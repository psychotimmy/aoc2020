      PROGRAM DAY21P2
C
      CHARACTER*800 LINE,INGLIST,ALGLIST,ANSWER
      CHARACTER*32 INGRLU(200),ALGLU(8),CDIL(2,8)
...
C
   ...
   45 FORMAT(2A)
C
      WRITE(*,10)"Advent of Code 2020 day 21, parts 1 and 2"
      WRITE(*,10)" "
C
...
C
C     To get the canonical dangerous ingredient list, create a
C     new lookup array from the allergens and ingredients lookups
      DO L1=1,NOALG
        CDIL(1,L1)=ALGLU(L1)
        CDIL(2,L1)=INGRLU(ALLERGENS(L1))
C       WRITE(*,*)CDIL(1,L1),CDIL(2,L1)
      ENDDO
C     Sort by allergen name (column 1)
      CALL INSORT(CDIL,NOALG)
C     WRITE(*,*)"----"
C     DO L1=1,NOALG
C       WRITE(*,*)CDIL(1,L1),CDIL(2,L1)
C     ENDDO
C     Create a comma separated list
      SEP=INDEX(CDIL(2,1),' ')
      ANSWER=CDIL(2,1)(1:SEP-1)
      ANSWERL=SEP-1
      DO L1=2,NOALG
        ANSWERL=ANSWERL+1
        ANSWER(ANSWERL:ANSWERL)=','
        SEP=INDEX(CDIL(2,L1),' ')
        ANSWER=ANSWER(1:ANSWERL)//CDIL(2,L1)(1:SEP-1)
        ANSWERL=ANSWERL+(SEP-1)
      ENDDO
      WRITE(*,*)" "
      WRITE(*,45)"Dangerous ingredient list is: ",ANSWER(1:ANSWERL)
      END
C
...
C     in ascending order
      CHARACTER*32 LIST(2,8),TEMP(2,1)
      INTEGER J,K,NUM
C
      DO J=2,NUM
        TEMP(1,1)=LIST(1,J)
        TEMP(2,1)=LIST(2,J)
        K=J-1
        DO WHILE ((LIST(1,K).GT.TEMP(1,1)).AND.(K.GE.1))
          LIST(1,K+1)=LIST(1,K)
          LIST(2,K+1)=LIST(2,K)
          K=K-1
        ENDDO
