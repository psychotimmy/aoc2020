...
C
   ...
   40 FORMAT(A,I16)
C
      WRITE(*,10)"Advent of Code 2020 day 18, part 1"
      WRITE(*,10)" "
C
...
C
      ...
      INTEGER TOKENS(*),NT,L1
C     Evaluate the expression from left to right - odd values will
C     be numbers, even values operators (+ or *)
      EVALUATE=INT8(TOKENS(1))
      DO L1=2,NT,2
        IF (TOKENS(L1).EQ.-100) THEN
          EVALUATE=EVALUATE+INT8(TOKENS(L1+1))
        ELSE IF (TOKENS(L1).EQ.-200) THEN
          EVALUATE=EVALUATE*INT8(TOKENS(L1+1))
        ENDIF
C       Note that the -500 token introduced when brackets are removed
C       in CALC is always ignored by this function.
      ENDDO
      RETURN
      END
